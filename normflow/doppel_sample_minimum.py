...
if __name__ == "__main__":
    import sys
    nfm_data = np.load(f"data/target2.npy")
    print(nfm_data)
    target_data = np.load("data/target.npy")
    print(target_data)

    config = {
        "store_name": f"small-store",
        "store_size": 512,
        "chunk_size": 10,
        "observation_path": None,
        "logratios_path": f"small-logratios",
        "trainer_dir": f"small-trainer",
    ...
    plt.savefig(f'viewer-small.png')
